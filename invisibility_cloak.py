...
#Allowing the camera to start by making the code sleep for 2 seconds
time.sleep(2)
bg=0
...
#Reading the captured frame untile the camera is open
while(cap.isOpened()):
    ret,img=cap.read()
    if not ret:
        break
    ...
    #Open and expand the image when there is a mask_1(color)
    mask_1=cv2.morphologyEx(mask_1,cv2.MORPH_OPEN,np.ones((3,3),np.uint8))
    mask_1=cv2.morphologyEx(mask_1,cv2.MORPH_DILATE,np.ones((3,3),np.uint8))
    
    #Selecting only the path where does not have mask_1 and saving in mask_2
    mask_2=cv2.bitwise_not(mask_1)

    #Keeping only the part of the images without the red color
    #Or anyother color u may choose
